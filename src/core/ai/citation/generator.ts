import { Paper, Author } from '../../types/paper.types'
import { CitationFormat } from '../../types/ai.types'

export class CitationGenerator {
  generate(paper: Paper, format: CitationFormat): string {
    switch (format) {
  ...
  generateAll(paper: Paper): Record<CitationFormat, string> {
    return {
...
// ── IEEE ─────────────────────────────────────────────────────────
function formatIEEE(p: Paper): string {
...
// ── Author Formatters ────────────────────────────────────────────
function lastFirstInitial(a: Author): string {
...
function firstInitialLast(a: Author): string {
  const parts = a.name.trim().split(' ')
  if (parts.length < 2) return a.name
  const last = parts.pop()!
