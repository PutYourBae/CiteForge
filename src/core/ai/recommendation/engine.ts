...
export class RecommendationEngine {
  async forPaper(paper: Paper, candidates?: Paper[]): Promise<Paper[]> {
  ...
  private scoreRelevance(source: Paper, candidate: Paper): number {
    let score = 0
    ...
    if (source.journal && candidate.journal &&
        source.journal.toLowerCase() === candidate.journal.toLowerCase()) score += 5
  ...
  private async fetchRelatedPapers(paper: Paper): Promise<Paper[]> {
    ...
    try {
      ...
      return (data?.recommendedPapers ?? []).map((r: any): Paper => {
