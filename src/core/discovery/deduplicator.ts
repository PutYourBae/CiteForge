...
function mergePapers(group: Paper[]): Paper {
  const sorted = [...group].sort(
    (a, b) => sourceWeight(b.sources[0]) - sourceWeight(a.sources[0])
  )
  const primary = sorted[0]
  const bestAbstract = group.reduce<string | undefined>((best, p) =>
    (p.abstract?.length ?? 0) > (best?.length ?? 0) ? p.abstract : best, primary.abstract)
