...
exports.CrossRefAdapter = void 0;
const base_adapter_1 = require("./base.adapter");
class CrossRefAdapter extends base_adapter_1.BaseAdapter {
    constructor() {
        super(...arguments);
    ...
    async search(query) {
        ...
        return (res.data?.message?.items ?? []).map((r) => this.mapWork(r));
    }
    mapWork(r) {
        const doi = this.normalizeDoi(r.DOI);
        const year = r.published?.['date-parts']?.[0]?.[0];
        const oaLink = (r.link ?? []).find((l) => l['content-type'] === 'application/pdf' && l['intended-application'] === 'text-mining');
        return {
...
exports.CrossRefAdapter = CrossRefAdapter;
