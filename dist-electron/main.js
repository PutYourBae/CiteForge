...
async function initDatabase() {
  ...
  const dbPath = path$1.join(userDataPath, "citeforge.db");
  db = new Database(dbPath);
  db.pragma("journal_mode = WAL");
  db.pragma("foreign_keys = ON");
  const schemaPath = path$1.join(__dirname, "../database/schema.sql");
...
function dispatchRequest(config) {
  ...
  const adapter = adapters$1.getAdapter(config.adapter || defaults.adapter, config);
...
class BaseAdapter {
  ...
  safeYear(raw) {
    ...
    return n >= 1e3 && n <= currentYear + 1 ? n : void 0;
  }
  normalizeDoi(raw) {
...
const BASE$2 = "https://api.semanticscholar.org/graph/v1";
const FIELDS = [
  "paperId",
  "title",
  "authors",
...
class SemanticScholarAdapter extends BaseAdapter {
  ...
  async search(query) {
    var _a;
    const res = await this.http.get(`${BASE$2}/paper/search`, {
      params: { query: query.text, fields: FIELDS, limit: 20, offset: 0 }
    });
    return (((_a = res.data) == null ? void 0 : _a.data) ?? []).map((r) => this.mapPaper(r));
  }
  mapPaper(r) {
    var _a, _b, _c, _d, _e;
    const doi = this.normalizeDoi((_a = r.externalIds) == null ? void 0 : _a.DOI);
    const isOA = !!((_b = r.openAccessPdf) == null ? void 0 : _b.url);
    return {
      id: doi ?? r.paperId,
      doi,
      ...
      journal: ((_c = r.journal) == null ? void 0 : _c.name) ?? ((_d = r.publicationVenue) == null ? void 0 : _d.name),
      ...
      pdfUrl: (_e = r.openAccessPdf) == null ? void 0 : _e.url,
      publisherUrl: doi ? `https://doi.org/${doi}` : `https://www.semanticscholar.org/paper/${r.paperId}`,
      sources: ["semantic_scholar"]
    };
  }
  mapType(types) {
    if (!(types == null ? void 0 : types.length)) return "unknown";
...
const BASE$1 = "https://api.openalex.org";
const SELECT = [
  "id",
  "doi",
  "title",
...
class OpenAlexAdapter extends BaseAdapter {
  ...
  async search(query) {
    var _a;
    const res = await this.http.get(`${BASE$1}/works`, {
      params: { search: query.text, select: SELECT, per_page: 20 },
      headers: { "User-Agent": "CiteForge/1.0 (mailto:citeforge@example.com)" }
    });
    return (((_a = res.data) == null ? void 0 : _a.results) ?? []).map((w) => this.mapWork(w));
  }
  mapWork(w) {
    var _a, _b, _c, _d, _e, _f, _g, _h;
    const doi = this.normalizeDoi(w.doi);
    const pdfUrl = ((_a = w.open_access) == null ? void 0 : _a.oa_url) ?? ((_b = w.primary_location) == null ? void 0 : _b.pdf_url);
    return {
      id: doi ?? w.id,
      doi,
      ...
      authors: (w.authorships ?? []).map((a) => {
        var _a2, _b2, _c2;
        return {
          name: ((_a2 = a.author) == null ? void 0 : _a2.display_name) ?? "Unknown",
          orcid: (_c2 = (_b2 = a.author) == null ? void 0 : _b2.orcid) == null ? void 0 : _c2.replace("https://orcid.org/", "")
        };
      }),
      abstract: this.truncate(this.reconstructAbstract(w.abstract_inverted_index)),
      year: this.safeYear(w.publication_year),
      journal: (_d = (_c = w.primary_location) == null ? void 0 : _c.source) == null ? void 0 : _d.display_name,
      citationCount: w.cited_by_count ?? 0,
      publicationType: this.mapType(w.type),
      accessStatus: ((_e = w.open_access) == null ? void 0 : _e.is_oa) ? "open_access" : doi ? "paid" : "unknown",
      pdfUrl: pdfUrl || void 0,
      publisherUrl: doi ? `https://doi.org/${doi}` : w.id,
      volume: (_f = w.biblio) == null ? void 0 : _f.volume,
      issue: (_g = w.biblio) == null ? void 0 : _g.issue,
      pages: ((_h = w.biblio) == null ? void 0 : _h.first_page) ? `${w.biblio.first_page}-${w.biblio.last_page ?? ""}`.replace(/-$/, "") : void 0,
      sources: ["openalex"]
    };
  }
  mapType(t) {
  ...
  parseAtomFeed(xml) {
    ...
    while ((match = entryRegex.exec(xml)) !== null) {
      try {
        ...
        const arxivId = ((_a = id.split("/abs/").pop()) == null ? void 0 : _a.replace(/v\d+$/, "")) ?? "";
        const title = (_b = this.extractTag(entry, "title")) == null ? void 0 : _b.trim();
        const abstract = (_c = this.extractTag(entry, "summary")) == null ? void 0 : _c.trim();
        ...
        papers.push({
          ...
          doi: doi ? this.normalizeDoi(doi) : void 0,
          arxivId,
...
class CrossRefAdapter extends BaseAdapter {
  constructor() {
    super(...arguments);
    this.sourceId = "crossref";
    this.displayName = "CrossRef";
  }
  async search(query) {
    var _a, _b;
    ...
    return (((_b = (_a = res.data) == null ? void 0 : _a.message) == null ? void 0 : _b.items) ?? []).map((r) => this.mapWork(r));
  }
  mapWork(r) {
    var _a, _b, _c, _d;
    const doi = this.normalizeDoi(r.DOI);
    const year = (_c = (_b = (_a = r.published) == null ? void 0 : _a["date-parts"]) == null ? void 0 : _b[0]) == null ? void 0 : _c[0];
    const oaLink = (r.link ?? []).find(
      (l) => l["content-type"] === "application/pdf" && l["intended-application"] === "text-mining"
    );
    return {
      id: doi ?? crypto.randomUUID(),
      doi,
      title: this.cleanTitle(r.title),
      authors: (r.author ?? []).map((a) => {
        var _a2;
        return {
          name: [a.given, a.family].filter(Boolean).join(" "),
          orcid: (_a2 = a.ORCID) == null ? void 0 : _a2.replace("http://orcid.org/", "")
        };
      }),
      abstract: this.truncate(((_d = r.abstract) == null ? void 0 : _d.replace(/<[^>]+>/g, "")) ?? ""),
      ...
      pdfUrl: oaLink == null ? void 0 : oaLink.URL,
  ...
  mapType(t) {
    ...
    if (t == null ? void 0 : t.includes("proceedings")) return "conference";
...
class PubMedAdapter extends BaseAdapter {
  ...
  async search(query) {
    ...
    try {
      ...
      const ids = ((_b = (_a = searchRes.data) == null ? void 0 : _a.esearchresult) == null ? void 0 : _b.idlist) ?? [];
      ...
      const uids = ((_d = (_c = summaryRes.data) == null ? void 0 : _c.result) == null ? void 0 : _d.uids) ?? [];
      const result = ((_e = summaryRes.data) == null ? void 0 : _e.result) ?? {};
  ...
  mapSummary(pmid, r) {
    ...
    const doi = this.normalizeDoi(
      (_b = (_a = r.articleids) == null ? void 0 : _a.find((a) => a.idtype === "doi")) == null ? void 0 : _b.value
    );
    const year = this.safeYear((_c = r.pubdate) == null ? void 0 : _c.split(" ")[0]);
    const authors = (r.authors ?? []).map((a) => ({ name: a.name }));
    const journal = r.fulljournalname ?? r.source;
    const hasPMC = (_d = r.articleids) == null ? void 0 : _d.some((a) => a.idtype === "pmc");
    const pmcId = (_f = (_e = r.articleids) == null ? void 0 : _e.find((a) => a.idtype === "pmc")) == null ? void 0 : _f.value;
    return {
      id: doi ?? `pmid:${pmid}`,
      doi,
      title: this.cleanTitle(r.title),
      authors,
      abstract: "",
      // ESummary doesn't include abstract, EFetch would be needed
      year,
      journal,
      volume: r.volume,
      issue: r.issue,
      ...
      pdfUrl: pmcId ? `https://www.ncbi.nlm.nih.gov/pmc/articles/${pmcId}/pdf/` : void 0,
      publisherUrl: doi ? `https://doi.org/${doi}` : `https://pubmed.ncbi.nlm.nih.gov/${pmid}/`,
      sources: ["pubmed"]
    };
  }
}
...
class CoreAdapter extends BaseAdapter {
  ...
  mapWork(w) {
    var _a, _b, _c;
    const doi = this.normalizeDoi(w.doi);
    const pdfUrl = w.downloadUrl ?? ((_a = w.sourceFulltextUrls) == null ? void 0 : _a[0]);
    return {
      id: doi ?? `core:${w.id}`,
      doi,
      title: this.cleanTitle(w.title),
      ...
      journal: ((_c = (_b = w.journals) == null ? void 0 : _b[0]) == null ? void 0 : _c.title) ?? w.publisher,
      ...
      pdfUrl: pdfUrl || void 0,
      publisherUrl: doi ? `https://doi.org/${doi}` : `https://core.ac.uk/works/${w.id}`,
      sources: ["core"]
    };
  }
}
...
const STOPWORDS = /* @__PURE__ */ new Set(["a", "an", "the", "of", "on", "in", "for", "and", "or", "to", "is"]);
function normalizeTitle(title) {
  return title.toLowerCase().replace(/[^a-z0-9\s]/g, "").split(/\s+/).filter((w) => !STOPWORDS.has(w)).join(" ");
}
...
function mergePapers(group) {
  const sorted = [...group].sort(
    (a, b) => sourceWeight(b.sources[0]) - sourceWeight(a.sources[0])
  );
  const primary = sorted[0];
  const bestAbstract = group.reduce((best, p) => {
    var _a;
    return (((_a = p.abstract) == null ? void 0 : _a.length) ?? 0) > ((best == null ? void 0 : best.length) ?? 0) ? p.abstract : best;
  }, primary.abstract);
  ...
  return {
    ...
    pdfUrl: (oaEntry == null ? void 0 : oaEntry.pdfUrl) ?? primary.pdfUrl,
    ...
    for (let i = 0; i < pass1.length; i++) {
      ...
      for (let j = i + 1; j < pass1.length; j++) {
        ...
        const sim = jaroWinkler(
          normalizeTitle(pass1[i].title),
          normalizeTitle(pass1[j].title)
        );
        if (sim >= 0.88) {
          group.push(pass1[j]);
          used.add(j);
...
class RelevanceRanker {
  ...
  score(p, q, maxCitations, currentYear) {
    return this.keywordScore(p, q.terms) * 0.4 + this.citationScore(p.citationCount, maxCitations) * 0.25 + this.yearScore(p.year, currentYear) * 0.2 + this.abstractScore(p.abstract, q.terms) * 0.15;
  }
  keywordScore(p, terms) {
    ...
    for (const term of terms) {
      ...
      else if ((_a = p.abstract) == null ? void 0 : _a.toLowerCase().includes(term)) score += 15;
      else if ((_b = p.journal) == null ? void 0 : _b.toLowerCase().includes(term)) score += 8;
    }
    if (terms.every((t) => titleL.includes(t))) score += 15;
    return Math.min(100, score);
  }
  citationScore(count, max2) {
    if (max2 === 0) return 0;
    return Math.log10(count + 1) / Math.log10(max2 + 1) * 100;
  }
...
class QueryProcessor {
  ...
  extractTerms(text) {
    ...
    return text.toLowerCase().replace(/[^a-z0-9\s]/g, " ").split(/\s+/).filter((w) => w.length > 2 && !stopwords.has(w)).slice(0, 8);
  }
}
const adapters = [
...
function registerSearchHandlers() {
  electron.ipcMain.handle("search:query", async (_e, query) => {
    var _a;
    ...
    const settled = await Promise.allSettled(
      adapters.map(
        (a) => Promise.race([
          a.search(processed),
          new Promise(
            (_, rej) => setTimeout(() => rej(new Error("timeout")), 8e3)
          )
        ])
      )
    );
    ...
    settled.forEach((result, i) => {
      ...
      if (result.status === "fulfilled") {
        ...
        const isTimeout = ((_a2 = result.reason) == null ? void 0 : _a2.message) === "timeout";
    ...
    const filtered = ranked.filter((p) => {
      var _a2, _b, _c;
      if (((_a2 = query.filters) == null ? void 0 : _a2.yearFrom) && (p.year ?? 0) < query.filters.yearFrom) return false;
      if (((_b = query.filters) == null ? void 0 : _b.yearTo) && (p.year ?? 9999) > query.filters.yearTo) return false;
      if (((_c = query.filters) == null ? void 0 : _c.accessType) === "open_access" && p.accessStatus !== "open_access") return false;
      return true;
    });
    return {
      papers: filtered.slice(0, ((_a = query.filters) == null ? void 0 : _a.maxResults) ?? 50),
...
function registerDownloadHandlers() {
  electron.ipcMain.handle("download:start", async (_e, paper) => {
    const db2 = getDb();
    const win = electron.BrowserWindow.getAllWindows()[0];
    const safeName = (paper.title ?? "paper").replace(/[^a-zA-Z0-9\s-]/g, "").trim().substring(0, 60).replace(/\s+/g, "_");
    const result = await electron.dialog.showSaveDialog(win, {
      title: "Save Paper PDF",
      defaultPath: path$1.join(electron.app.getPath("downloads"), `${safeName}.pdf`),
    ...
    `).run({
      ...
      year: paper.year ?? (/* @__PURE__ */ new Date()).getFullYear(),
    ...
    const onWillDownload = (e, item, webContents) => {
      ...
      item.on("updated", (event, state2) => {
        if (state2 === "interrupted") {
          try {
            db2.prepare(`UPDATE downloads SET status='failed', error_msg=? WHERE id=?`).run("Interrupted", dlId);
            win.webContents.send("download:progress", { downloadId: dlId, status: "failed", error: "Network interrupted" });
          } catch {
          }
        } else if (state2 === "progressing") {
          if (!item.isPaused()) {
            const received = item.getReceivedBytes();
            const total = item.getTotalBytes();
            const progress = total > 0 ? received / total : 0.5;
            try {
              db2.prepare("UPDATE downloads SET progress=? WHERE id=?").run(progress, dlId);
      ...
      item.once("done", (event, state2) => {
        if (state2 === "completed") {
          try {
            db2.prepare(`UPDATE downloads SET status='done', progress=1.0, completed_at=datetime('now') WHERE id=?`).run(dlId);
            win.webContents.send("download:progress", { downloadId: dlId, progress: 1, status: "done", filePath });
          } catch {
          }
        } else {
          try {
            db2.prepare(`UPDATE downloads SET status='failed', error_msg=? WHERE id=?`).run(state2, dlId);
            win.webContents.send("download:progress", { downloadId: dlId, status: "failed", error: state2 });
          } catch {
          }
        }
      });
  ...
  electron.ipcMain.handle("download:cancel", (_e, downloadId) => {
    getDb().prepare(`UPDATE downloads SET status='cancelled' WHERE id=?`).run(downloadId);
    return { success: true };
  });
  electron.ipcMain.handle(
    "download:list",
    () => getDb().prepare("SELECT * FROM downloads ORDER BY started_at DESC LIMIT 50").all()
  );
}
function registerDatabaseHandlers() {
  electron.ipcMain.handle("db:savePaper", async (_e, paper) => {
    try {
      const db2 = getDb();
      const saveTx = db2.transaction(() => {
        const stmt = db2.prepare(`
        ...
        if (paper.authors && paper.authors.length > 0) {
          const insertAuthor = db2.prepare(`
            INSERT OR IGNORE INTO authors (id, name, orcid) VALUES (?, ?, ?)
          `);
          const insertPaperAuthor = db2.prepare(`
  ...
  electron.ipcMain.handle("db:getSavedPapers", () => {
    const db2 = getDb();
  ...
  electron.ipcMain.handle("db:deleteSavedPaper", (_e, id) => {
    getDb().prepare("DELETE FROM saved_papers WHERE paper_id = ?").run(id);
    return { success: true };
  });
  electron.ipcMain.handle("db:updatePaperNotes", (_e, id, notes) => {
    getDb().prepare("UPDATE saved_papers SET notes = ? WHERE paper_id = ?").run(notes, id);
    return { success: true };
  });
  electron.ipcMain.handle("db:getAIInsight", (_e, paperId) => {
  ...
  electron.ipcMain.handle("db:saveAIInsight", (_e, paperId, insight) => {
    ...
    `).run(
      paperId,
      (_a = insight.summaries) == null ? void 0 : _a.short,
      (_b = insight.summaries) == null ? void 0 : _b.research,
      JSON.stringify(((_c = insight.summaries) == null ? void 0 : _c.keyContributions) ?? []),
      (_d = insight.summaries) == null ? void 0 : _d.methodOverview,
      JSON.stringify(insight.topics ?? []),
      (_e2 = insight.citations) == null ? void 0 : _e2.apa,
      (_f = insight.citations) == null ? void 0 : _f.ieee,
      (_g = insight.citations) == null ? void 0 : _g.mla,
      (_h = insight.citations) == null ? void 0 : _h.chicago,
      (_i = insight.citations) == null ? void 0 : _i.bibtex,
      ((_j = insight.summaries) == null ? void 0 : _j.generatedBy) ?? "local_rules"
    );
    return { success: true };
  });
  ...
  electron.ipcMain.handle("db:getSetting", (_e, key) => {
    const row = getDb().prepare(
      "SELECT value FROM settings WHERE key = ?"
    ).get(key);
    return row ? JSON.parse(row.value) : null;
  });
  electron.ipcMain.handle("db:saveSettings", (_e, settings) => {
    const db2 = getDb();
    const stmt = db2.prepare(
      "INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)"
    );
    const saveMany = db2.transaction((s) => {
...
function normalizePaperRow(row) {
  return {
    ...
    authors: [],
    // Authors loaded separately
    tags: row.tags ? JSON.parse(row.tags) : [],
    notes: row.notes,
    savedAt: row.savedAt
    // alias from SELECT AS
  };
}
function registerSystemHandlers() {
  electron.ipcMain.handle("system:openExternal", async (_e, url2) => {
    if (!url2 || !url2.startsWith("http")) return;
    await electron.shell.openExternal(url2);
  });
  electron.ipcMain.handle("system:openFolder", async (_e, folderPath) => {
    await electron.shell.openPath(folderPath);
  });
  electron.ipcMain.handle("system:showItemInFolder", async (_e, filePath) => {
    electron.shell.showItemInFolder(filePath);
  });
  electron.ipcMain.handle("system:saveDialog", async (_e, defaultName) => {
    const result = await electron.dialog.showSaveDialog({
      defaultPath: path$1.join(electron.app.getPath("downloads"), defaultName),
      filters: [
  ...
  electron.ipcMain.handle("system:folderDialog", async () => {
  ...
  electron.ipcMain.handle("system:getVersion", () => electron.app.getVersion());
  electron.ipcMain.handle("system:getPlatform", () => process.platform);
  electron.ipcMain.handle("system:getDownloadsPath", () => electron.app.getPath("downloads"));
}
class ContextBuilder {
  build(paper, pdf) {
    ...
    return {
      paper,
      abstract,
      fullText: pdf == null ? void 0 : pdf.fullText,
      sections: pdf == null ? void 0 : pdf.sections,
      keyTerms,
      pdfAvailable: !!pdf
    };
  }
  extractKeyTerms(text) {
    ...
    return Array.from(freq.entries()).sort((a, b) => b[1] - a[1]).slice(0, 15).map(([word]) => word);
  }
}
class ExtractiveSummarizer {
  ...
  summarize(text, sentenceCount = 3) {
    ...
    const topIndices = scores.map((score, i) => ({ score, i })).sort((a, b) => b.score - a.score).slice(0, sentenceCount).map((x) => x.i).sort((a, b) => a - b);
  ...
  splitSentences(text) {
    return text.replace(/\n+/g, " ").split(new RegExp("(?<=[.!?])\\s+(?=[A-Z])")).map((s) => s.trim()).filter((s) => s.length > 20 && s.length < 500);
  }
  tfVector(sentence) {
...
class LLMRouter {
  async summarize(ctx) {
    ...
    const text = ctx.abstract.substring(0, 3e3);
    if (!text) return null;
  ...
  async callGemini(text, apiKey) {
    ...
    const summary = ((_e = (_d = (_c = (_b = (_a = data == null ? void 0 : data.candidates) == null ? void 0 : _a[0]) == null ? void 0 : _b.content) == null ? void 0 : _c.parts) == null ? void 0 : _d[0]) == null ? void 0 : _e.text) ?? "";
  ...
  async callOpenAI(text, apiKey) {
    ...
    const data = await res.json();
    const summary = ((_c = (_b = (_a = data == null ? void 0 : data.choices) == null ? void 0 : _a[0]) == null ? void 0 : _b.message) == null ? void 0 : _c.content) ?? "";
...
class SummaryEngine {
  async generate(ctx) {
    var _a, _b, _c, _d;
    const sourceText = ctx.fullText ? `${((_a = ctx.sections) == null ? void 0 : _a.abstract) ?? ctx.abstract}

${((_b = ctx.sections) == null ? void 0 : _b.introduction) ?? ""}

${((_c = ctx.sections) == null ? void 0 : _c.conclusion) ?? ""}` : ctx.abstract;
    if (!sourceText.trim()) {
      return {
        short: "No abstract available for this paper.",
        research: void 0,
        keyContributions: [],
        methodOverview: void 0,
        isLoading: false,
    ...
    const methodOverview = ((_d = ctx.sections) == null ? void 0 : _d.methodology) ? extractive.summarize(ctx.sections.methodology, 2) : void 0;
    let research;
...
function authorsMLA(authors) {
  if (!authors.length) return "Unknown";
  if (authors.length === 1) return lastFirstInitial(authors[0]).replace(/,\s+(\w)\.$/, ", $1");
  if (authors.length === 2) {
...
function sentenceCase(title) {
  return title.charAt(0).toUpperCase() + title.slice(1).toLowerCase().replace(/:\s*(\w)/g, (_, ch) => ": " + ch.toUpperCase());
}
class GraphEngine {
  async buildGraph(centerPaper) {
    ...
    for (const ref2 of references.slice(0, 8)) {
      if (!seen.has(ref2.id)) {
        nodes.push(this.paperToNode(ref2, false));
        edges.push({
  ...
  async fetchLinks(paper, type2) {
    ...
    try {
      ...
      const res = await fetch(
        `https://api.semanticscholar.org/graph/v1/paper/${encodeURIComponent(id)}/${type2}?fields=${fields}&limit=10`,
      ...
      return items.map((item) => {
        var _a, _b, _c;
        const p = type2 === "references" ? item.citedPaper : item.citingPaper;
        if (!p) return null;
        const doi = (_b = (_a = p.externalIds) == null ? void 0 : _a.DOI) == null ? void 0 : _b.toLowerCase();
        return {
          ...
          pdfUrl: (_c = p.openAccessPdf) == null ? void 0 : _c.url,
          publisherUrl: doi ? `https://doi.org/${doi}` : "#",
          sources: ["semantic_scholar"]
        };
      }).filter(Boolean);
...
const TOPIC_TAXONOMY = {
  "Machine Learning": ["machine learning", "neural", "deep learning", "training", "model", "classifier", "regression", "supervised", "unsupervised", "gradient", "backpropagation"],
  "NLP": ["natural language", "text", "nlp", "language model", "bert", "gpt", "transformer", "sentiment", "named entity", "tokenization", "embedding", "word2vec"],
  "Computer Vision": ["image", "vision", "convolutional", "cnn", "object detection", "segmentation", "recognition", "pixel", "feature extraction", "yolo", "resnet"],
  "Reinforcement Learning": ["reinforcement", "reward", "agent", "policy", "q-learning", "markov", "environment", "exploration"],
  "Data Science": ["data", "dataset", "pandas", "statistics", "visualization", "analysis", "preprocessing", "feature engineering", "correlation"],
  "Cybersecurity": ["security", "attack", "defense", "vulnerability", "malware", "intrusion detection", "encryption", "authentication", "adversarial"],
  "Bioinformatics": ["genome", "protein", "dna", "rna", "sequence", "bioinformatics", "genomics", "phylogenetic", "molecular"],
  "Healthcare & Medicine": ["clinical", "medical", "patient", "disease", "diagnosis", "treatment", "hospital", "drug", "health", "biomarker"],
  "Renewable Energy": ["renewable", "solar", "wind", "battery", "energy storage", "photovoltaic", "climate", "carbon", "emission", "sustainability"],
  "IoT & Edge Computing": ["iot", "internet of things", "edge computing", "embedded", "sensor", "microcontroller", "smart", "fog computing"],
  "Blockchain": ["blockchain", "distributed ledger", "smart contract", "cryptocurrency", "decentralized", "consensus", "ethereum", "bitcoin"],
  "Optimization": ["optimization", "genetic algorithm", "evolutionary", "metaheuristic", "swarm", "particle", "simulated annealing", "heuristic"],
  "Cloud Computing": ["cloud", "serverless", "microservice", "container", "kubernetes", "docker", "scaling", "distributed system"],
  "Graph Theory": ["graph", "network", "node", "edge", "topology", "community detection", "centrality", "knowledge graph"],
  "Education Technology": ["learning", "education", "student", "curriculum", "e-learning", "gamification", "classroom", "pedagogy"]
};
class TopicDiscovery {
  detect(ctx) {
    ...
    const text = [
      ctx.paper.title,
      ctx.abstract,
      ((_a = ctx.fullText) == null ? void 0 : _a.substring(0, 3e3)) ?? ""
    ].join(" ").toLowerCase();
    const scores = {};
    ...
    return Object.entries(scores).filter(([, score]) => score >= 2).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([topic]) => topic);
  }
}
class RecommendationEngine {
  async forPaper(paper, candidates) {
    ...
    const scores = candidates.filter((c) => c.id !== paper.id).map((c) => ({
      paper: c,
  ...
  scoreRelevance(source, candidate) {
    ...
    const union = /* @__PURE__ */ new Set([...srcTerms, ...cndTerms]);
    const jaccard = union.size > 0 ? intersection.size / union.size : 0;
    score += jaccard * 50;
    if (candidate.citationCount > 50) score += 10;
    if (candidate.citationCount > 500) score += 15;
    const currentYear = (/* @__PURE__ */ new Date()).getFullYear();
    if (candidate.year && currentYear - candidate.year <= 5) score += 20;
    else if (candidate.year && currentYear - candidate.year <= 10) score += 10;
    if (source.journal && candidate.journal && source.journal.toLowerCase() === candidate.journal.toLowerCase()) score += 5;
    if (candidate.accessStatus === "open_access") score += 5;
    return Math.round(score);
  }
  extractTerms(text) {
    const stopwords = /* @__PURE__ */ new Set(["a", "an", "the", "of", "on", "in", "for", "and", "or", "to", "is", "are"]);
    const terms = text.toLowerCase().replace(/[^a-z0-9\s]/g, " ").split(/\s+/).filter((w) => w.length > 3 && !stopwords.has(w));
    return new Set(terms);
  }
  async fetchRelatedPapers(paper) {
    if (!paper.doi && !paper.arxivId) return [];
    try {
      ...
      const res = await fetch(
        `https://api.semanticscholar.org/graph/v1/paper/${encodeURIComponent(id)}/recommendations?fields=${fields}&limit=15`,
        { signal: AbortSignal.timeout(8e3) }
      );
      if (!res.ok) return [];
      const data = await res.json();
      return ((data == null ? void 0 : data.recommendedPapers) ?? []).map((r) => {
        var _a, _b, _c;
        const doi = (_b = (_a = r.externalIds) == null ? void 0 : _a.DOI) == null ? void 0 : _b.toLowerCase();
        return {
          id: doi ?? r.paperId,
          doi,
          ...
          pdfUrl: (_c = r.openAccessPdf) == null ? void 0 : _c.url,
...
class PDFExtractor {
  async extract(url2) {
    const pdfjsLib = await Promise.resolve().then(() => require("./pdf-DnKROPZf.js"));
    ...
    const pagesToExtract = totalPages > 30 ? this.getKeyPageIndices(totalPages) : Array.from({ length: totalPages }, (_, i) => i + 1);
    const pageTexts = [];
  ...
  detectSections(text) {
    ...
    for (const [section, regex] of Object.entries(markers)) {
      ...
      const sectionText = text.substring(matchIdx, matchIdx + 2e3).replace(/^.*?(abstract|introduction|methodology|methods|results|conclusion)/i, "").trim();
...
class AIIntelligenceEngine {
  async enrich(paper) {
    const pdfData = paper.pdfUrl && paper.accessStatus === "open_access" ? await pdfExtractor.extract(paper.pdfUrl).catch(() => void 0) : void 0;
...
function registerAIHandlers() {
  electron.ipcMain.handle("ai:enrich", async (_e, paper) => {
    var _a, _b, _c, _d, _e2;
    try {
      const db2 = getDb();
      ...
      `).run(
        ...
        ((_d = insight.summaries) == null ? void 0 : _d.methodOverview) ?? "",
        JSON.stringify(insight.topics ?? []),
        ((_e2 = insight.summaries) == null ? void 0 : _e2.generatedBy) ?? "local_rules"
      );
      return insight;
    } catch (err) {
  ...
  electron.ipcMain.handle("ai:cite", async (_e, paper, format) => {
    return aiEngine.cite(paper, format);
  });
  electron.ipcMain.handle("ai:citeAll", async (_e, paper) => {
      ...
      submenu: [
        {
          ...
          click: () => mainWindow2.webContents.send("menu:new-search")
        },
        { type: "separator" },
      ...
      submenu: [
        {
          ...
          click: () => electron.shell.openExternal("https://github.com/your-username/CiteForge")
        },
        {
          label: "Report Issue",
          click: () => electron.shell.openExternal("https://github.com/your-username/CiteForge/issues")
        },
        { type: "separator" },
        { label: `CiteForge v${electron.app.getVersion()}`, enabled: false }
      ]
    }
  ];
...
function createWindow() {
  mainWindow = new electron.BrowserWindow({
    ...
    icon: path$1.join(__dirname, "../assets/icon.ico"),
...
electron.app.on("before-quit", () => closeDatabase());
